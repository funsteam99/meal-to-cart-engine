...
Use the deployed Cloud Run URL as the source of truth:
...
1. Open the deployed app.
2. Click `Use Demo Ingredients`.
3. Open `Recipe`.
4. Click `Regenerate Recipe`.
5. Confirm the status says `Generated by the configured model.`
6. Confirm the recipe options are not the local fallback set.
...
- Chinese Markdown files are saved as UTF-8 with BOM so Windows PowerShell can read them correctly.
- Use the GCP Cloud Run URL for demo validation; avoid mixing results with localhost.
